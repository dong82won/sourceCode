...
class FloorGeneratorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("가제보 SDF 바닥 자동 생성기 (실시간 자석 스냅 & 미세조각 방지)")
    ...
    def setup_ui(self):
        ...
        info_label = tk.Label(top_frame, text="2. 드래그하여 바닥 생성 (벽이나 기존 바닥 근처에 마우스를 대면 찰칵! 달라붙습니다)", fg="blue")
        info_label.pack()
    ...
    def on_mouse_down(self, event):
        # 마우스를 누를 때 모든 벽과 바닥의 좌표를 수집해둠 (실시간 스냅용)
        self.snap_x_targets = []
        self.snap_y_targets = []
        for r in self.confirmed_floors:
            self.snap_x_targets.extend([r[0], r[2]])
            self.snap_y_targets.extend([r[1], r[3]])
            
        for item in self.canvas.find_withtag("wall"):
            coords = self.canvas.coords(item)
            if len(coords) == 4:
                self.snap_x_targets.extend([coords[0], coords[2]])
                self.snap_y_targets.extend([coords[1], coords[3]])
    ...
    def on_mouse_drag(self, event):
        # 드래그 중에도 빨간색 점선이 벽에 찰칵 달라붙도록 실시간 처리
        cur_x = self.get_snapped_coord(event.x, self.snap_x_targets)
        cur_y = self.get_snapped_coord(event.y, self.snap_y_targets)
    ...
    def on_mouse_up(self, event):
        ...
        if abs(cur_x - self.start_x) < 10 or abs(cur_y - self.start_y) < 10: 
            if self.rect_id: self.canvas.delete(self.rect_id)
            return
        ...
        for r in rects_to_process:
            # 1px짜리 미세 조각이 불필요하게 쪼개지는 현상을 막기 위해 필터 기준을 5px로 상향
