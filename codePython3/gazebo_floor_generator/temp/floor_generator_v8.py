...
class FloorGeneratorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Gazebo SDF Floor Master v8 (Project Export)")
    ...
    def setup_ui(self):
        ...
        tk.Button(left_btn_frame, text="[열기] model.sdf", command=self.load_sdf, font=("Arial", 10, "bold")).pack(side=tk.LEFT, padx=(5, 5))
        ...
        help_text = "좌클릭: 바닥 생성  |  우클릭: 삭제  |  휠: 줌  |  휠클릭(드래그): 이동"
        ...
        # 버튼 배정
        tk.Button(bottom_frame, text="새 폴더에 프로젝트 저장", command=self.export_project, bg="#2196F3", fg="white", font=("Arial", 9, "bold")).pack(side=tk.RIGHT, padx=5)
        tk.Button(bottom_frame, text="이미지 저장", command=self.save_canvas_image, bg="#607D8B", fg="white", font=("Arial", 9, "bold")).pack(side=tk.RIGHT, padx=5)
    ...
    def export_project(self):
        ...
        # 1. 상위 폴더 선택 (parent 옵션 추가)
        base_dir = filedialog.askdirectory(
            title="프로젝트 폴더를 생성할 위치를 선택하세요", 
            parent=self.root  # 메인 창 위로 강제 고정
        )
        if not base_dir: return

        # 2. 이름 입력 창 띄우기 (parent 옵션 추가)
        project_name = simpledialog.askstring(
            "새 폴더 생성", 
            "생성할 프로젝트(폴더) 이름을 입력하세요:", 
            initialvalue="My_Floor_Project",
            parent=self.root  # 메인 창 위로 강제 고정
        )
        
        # 사용자가 취소를 누르거나 빈 칸으로 둔 경우 취소 처리
        if not project_name:
            return
        ...
        try:
            ...
            messagebox.showinfo("성공", f"새 폴더가 생성되고 프로젝트가 성공적으로 저장되었습니다!\n\n저장 위치: {full_project_path}")
    ...
    def save_canvas_image(self):
        if not self.confirmed_floors and not self.walls_data:
        ...
        self.save_image_logic(save_path)
        messagebox.showinfo("완료", "이미지가 저장되었습니다.")

    def save_image_logic(self, file_path):
        """캔버스 내용을 PIL 이미지를 사용하여 파일로 저장합니다."""
        if Image is None:
            messagebox.showwarning("Pillow 미설치", "이미지 저장을 위해 'pip install Pillow'가 필요합니다.")
            return

        # 캔버스 크기의 새 이미지 생성
        img = Image.new("RGB", (self.canvas_w, self.canvas_h), "#f9f9f9")
