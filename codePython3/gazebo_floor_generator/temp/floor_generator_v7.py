...
class FloorGeneratorApp:
    ...
    def setup_ui(self):
        ...
        tk.Button(left_btn_frame, text="model.sdf 열기", command=self.load_sdf, font=("Arial", 10, "bold")).pack(side=tk.LEFT, padx=(5, 5))
        ...
        # 조작법 안내
        help_text = "좌클릭: 바닥 생성  |  우클릭: 조각 삭제  |  휠: 줌(확대/축소)  |  휠클릭(드래그): 화면 이동"
        ...
        tk.Button(bottom_frame, text="SDF파일 추가 저장", command=self.save_to_file, bg="#2196F3", fg="white", font=("Arial", 9, "bold")).pack(side=tk.RIGHT, padx=5)
    ...
    def draw_workspace(self):
        ...
        # 바닥 (텍스트 추가)
        # 이전 단계에서 SDF 코드 번호와 맞추기 위해 enumerate(..., start=1)을 사용합니다.
        for idx, f in enumerate(self.confirmed_floors, start=1):
            ...
            center_y = (sy1 + sy2) / 2
            # 3. 텍스트 추가
            self.canvas.create_text(center_x, center_y, text=f"Floor_{idx}", font=("Arial", 11, "bold"), fill="#333333", tags="floor_text")
    ...
    def on_mouse_up(self, event):
        ...
        for r in rects_to_process:
            # 💡 [핵심 수정] 0.05(5cm) -> 0.2(20cm)로 상향 조정
            # 이렇게 하면 겹친 바닥 위를 드래그할 때 생기는 미세한 찌꺼기 테두리가 무시됩니다.
        ...
        # 💡 [추가 및 수정된 부분] added 상태에 따라 분기 처리
        if added:
            self.color_index += 1
        else:
            # 바닥이 추가되지 않았다면 알림창을 띄웁니다.
            messagebox.showinfo("알림", "이미 바닥이 꽉 채워져 있거나 영역이 너무 작습니다.")
    ...
    def get_sdf_string(self):
        code = ""
        # 💡 [핵심 수정] enumerate를 사용해 실제 화면에 남아있는 바닥 개수만큼만 1번부터 순차적으로 번호(idx)를 부여합니다.
        for idx, f in enumerate(self.confirmed_floors, start=1):
            _, x1, y1, x2, y2, _, material = f
            gx, gy = round((x1+x2)/2, 3), round((y1+y2)/2, 3)
            gw, gh = round(x2-x1, 3), round(y2-y1, 3)

            # Floor_fid 대신 Floor_idx를 사용합니다.
